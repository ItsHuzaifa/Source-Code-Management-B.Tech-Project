...
class Action
{
	...
	function save_user()
	{
		...
		// To check if file already exist in database or not
		if (empty($id)) {
		...
		if ($save) {
			return 1;	// 1 always indicates true and 0 indicates false 
		}
	}
	function signup()
	{
	...
	function save_project()
	{
		...
		// move zip file in assets folder
		if (isset($_FILES['file']) && $_FILES['file']['tmp_name'] != '') {
			$fname = strtotime(date('y-m-d H:i')) . '_' . $_FILES['file']['name']; //fname = time + scm.zip
